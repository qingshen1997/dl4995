...
transform = transforms.Compose(
    ...
    torchvision.transforms.RandomRotation(30, resample=False, expand=False, center=None),transforms.ToTensor(),
...
#img = io.imread("0LBIWV.jpg")
img = Image.open("0LBIWV.jpg")
img = np.array(img)
noise_gs_img = util.random_noise(img,mode='gaussian')
plt.subplot(2,1,1), plt.title('original')
plt.imshow(img)
plt.subplot(2,1,2),plt.title('gaussian')
...
data=pd.DataFrame(columns=['ID'],data=names[:610])

data.to_csv(r"/Users/qingshen/Desktop/20spring/4995DL/submissionfile.csv",index="False")'''
